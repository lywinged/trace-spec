...
| MUST use an RFC 8785-conformant library | Implementation (both sides) | `examples/canonicalization-boundary/` — three schema-valid records on which every ad-hoc serializer diverges from RFC 8785; `test_canonicalization_boundary.py` recomputes each vector's declared divergence | The IEEE 754 half of the warning is unreachable: no schema field is typed `number`, and a test pins that fact. Before these vectors existed, nothing observable distinguished a conformant canonicalizer from `json.dumps`. |
