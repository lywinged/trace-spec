...
NO_ARGUMENT_TO_SWEEP = {
    ...
    # Takes nothing. It reads the packaged schema files and returns the eat_profile
    # URIs they declare, so there is no argument a caller could get wrong. Its own
    # failure mode is a build with no usable schema, which it raises RuntimeError for
    # and which `test_sign.py` covers.
