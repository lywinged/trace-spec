...
_RAW = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("TRACE_TESTS", "")
...
if not _RAW:
    sys.exit(
        "no trace-tests checkout given. Pass it as argv[1] or set TRACE_TESTS. Exiting "
        "non-zero rather than defaulting to the current directory, which would compare "
        "an unrelated tree and report agreement."
    )
