...
```bash
git clone https://github.com/agentrust-io/trace-tests
cd trace-tests && pip install -e ".[dev]" && pytest -q     # expect 118 passed, 5 xpassed

python scripts/mutate_modules.py /path/to/trace-tests      # exits 1 if any check is unverified
python scripts/enum_drift.py     /path/to/trace-tests      # exits 1 on any drifted enum
```

Both take the checkout as `argv[1]` or from `TRACE_TESTS`, and both exit non-zero rather
than reporting success over a corpus they could not find.
