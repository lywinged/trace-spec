...
`examples/verifier-compatibility/`, ten fixtures, run by
...
silently. Two of the ten, 04 and 09, carry a `preconditions` block stating what has
...
verifier cannot support a profile whose shape it cannot check, so a URI whose schema the
verifier does not carry MUST NOT appear in its declared set, whatever the verifier's documentation says
